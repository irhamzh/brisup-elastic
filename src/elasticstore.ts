...
export default class Elasticstore {
  ...
  initElasticsearch = async () => {
    return new Promise((resolve, reject) => {
      ...
      console.log(
        colors.grey("Connecting to ElasticSearch host %s:%s"),
        Config.ES_HOST,
        Config.ES_PORT
      );
      let retries = 0;
      this.retryTimer = setInterval(async () => {
        try {
          await this.ping();
          console.log(
            colors.green("Connected to ElasticSearch host %s:%s"),
            Config.ES_HOST,
            Config.ES_PORT
          );
    ...
    try {
      ...
      admin.initializeApp({
        credential: admin.credential.cert(
          Config.FB_SERVICE_ACCOUNT
            ? JSON.parse(Config.FB_SERVICE_ACCOUNT)
            : Config.FB_SERVICE_PATH
        ),
        databaseURL: Config.FB_URL,
      });
      console.log(
        colors.green("Connected to Firebase url:"),
        Config.FB_URL
      );
    } catch (e) {
